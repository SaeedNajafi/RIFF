...
function install_ml_libraries () {
	if [ "$OS" = "mac" ]; then
		...
		python3.9 -m pip install tensorflow-gpu==2.11.0
...
function install_reference_methods () {
	if [ "$ENV_NAME" = "prompt_torch" ]; then
		python3.9 -m pip install transformers datasets sentencepiece nltk evaluate bert-score supar pandas scikit-learn
